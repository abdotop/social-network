    ...
    <div class="max-w-[1065px] mx-auto">
      ...
      <div class="bg-white shadow lg:rounded-b-2xl lg:-mt-10 dark:bg-dark2">
        ...
        <div class="flex items-center justify-between border-t border-gray-100 px-2 dark:border-slate-700">
          ...
          <nav
            ...
            <button @click="navigate('friends')" class="inline-block py-3 leading-8 px-3.5"
              :class="{ 'border-b-2 border-blue-600 text-blue-600': status === 'friends' }">
              Follow
            </button>
          </nav>
      ...
      <div class="flex 2xl:gap-12 gap-10 mb-8 max-lg:flex-col" id="js-oversized">
        ...
        <div v-if="status === 'friends'">
          <Friends :status="friends" :data="data"/>
        </div>
      </div>
    </div>
...
export default {
  ...
  async mounted() {
    ...
    this.data.followers = await GetAllFollower()
    this.data.followees = await GetAllFollowee()
  },
  methods: {
    async navigate(to) {
